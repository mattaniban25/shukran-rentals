...
if($_SERVER['REQUEST_METHOD'] == 'GET'){
    ...
    $id = $_GET["id"];
    $roomTypeSql = "SELECT * FROM roomtype WHERE id = $id";
    $roomTypeResult = $conn->query($roomTypeSql);
    $row = $roomTypeResult->fetch_assoc();
    ...
    $roomMaxCapacity = $row["roomMaxCapacity"];
    ...
    do{
        ...
        if($rowCount > 0){
            $roomTypeCheckSql = "SELECT * FROM roomtype WHERE id = '$id'";
            $roomTypeCheckResult = mysqli_query($conn, $roomTypeCheckSql);
            $row = $roomTypeCheckResult->fetch_assoc();
            $roomTypeCheck = $row["roomType"];

            if($roomTypeCheck == $roomType){
                $value = $roomDescription;
                $new_value = str_replace("'", "''", "$value");

                $roomTypeSql="UPDATE roomtype SET roomType = '$roomType', roomDescription = '$new_value', roomCapacity = '$roomCapacity', roomMaxCapacity = '$roomMaxCapacity', roomPrice = '$roomPrice' WHERE id = $id";
...
<body>
    ...
    <main class="staff-content">
      <div class="border border-4 border-primary-subtle register-container rounded-3 p-4  bg-white">
        <br>
          <p class="fs-5" align="center"><i class= "fi fi-rs-bed-alt align-middle pe-3"></i>Edit Room Type</p>
          ...
          <form method="post">
            ...
            <?php
              ...
              <div class="row">
                  <div class="form-group col-lg-6 col-md-6 col-sm-12">
                      <button type="submit" class="btn btn-success btn-sm" name="submit">Edit Room Type</button>
